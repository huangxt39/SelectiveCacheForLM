...
parser.add_argument('--fix_cache_size', type=int, default=0)
parser.add_argument('--new_fix_cache_per_step', type=int, default=0)
parser.add_argument('--bp_cache_size', type=int, default=0)
parser.add_argument('--bp_cache_num', type=int, default=0)
...
parser.add_argument('--replacer', action='store_true')
...
if not args.replacer:
    model = T5ModelDecoderCacheSelector.from_pretrained(model_name, block_size=args.block_size, xl_cache_size=args.xl_cache_size, fix_cache_size=args.fix_cache_size, \
                                    threshold=args.threshold, new_fix_cache_per_step=args.new_fix_cache_per_step, selector_cache_size=args.bp_cache_size, \
                                    selector_cache_num=args.bp_cache_num, rmD=args.rmD, L0_lambda=args.L0_lambda, tokenizer=tokenizer)
else:
    model = T5ModelDecoderCacheReplacer.from_pretrained(model_name, block_size=args.block_size, xl_cache_size=args.xl_cache_size, fix_cache_size=args.fix_cache_size, \
                                     selector_cache_size=args.bp_cache_size, \
                                    selector_cache_num=args.bp_cache_num, tokenizer=tokenizer)
...
data_path = f'./LMdatasets/processed-wiki2-{tokenizer_name}-{batch_size}-{block_size}-fix12'
if os.path.exists(data_path) and not args.regen_data:
    print('loading dataset...')
    tokenized_dataset = load_from_disk(data_path)
else:
    tokenized_dataset = process_wiki(batch_size, block_size, tokenizer)
    ...
    for split in tokenized_dataset.keys():
        bs = batch_size if split == 'train' else 8
        ...
        with torch.no_grad():
            for s in tqdm(range(0, len(input_ids), bs)):
                ...
                # fix_duration.extend(torch.round(pred).clamp(min=0, max=fix_max).long().tolist())
...
per_device_batch_size_eval = 8 // num_device
assert per_device_batch_size_eval * num_device == 8
...
training_args = TrainingArguments(
    ...
    eval_delay=10 if 'base' in model_name else 25,
    num_train_epochs=40 if 'base' in model_name else 50,
    save_strategy="epoch",
    save_total_limit=3,
