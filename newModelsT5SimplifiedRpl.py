...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=False):
        super().__init__()
        ...
        assert fix_cache_size == block_size
    ...
    def init_fix_cache(self, batch_size_per_device):
        for i in range(torch.cuda.device_count()):
            device = torch.device(f'cuda:{i}')
            shape = (batch_size_per_device, self.n_heads, self.fix_cache_size, self.key_value_proj_dim)

            k = torch.zeros(shape, device=device)
            v = torch.zeros(shape, device=device)
            h = torch.zeros(batch_size_per_device, self.fix_cache_size, self.d_model, device=device)
            d = torch.full(shape[:-1], -1, device=device)
            
            id = torch.full(shape[:-1], 0, device=device)
            t = torch.full((batch_size_per_device, self.fix_cache_size), 0, device=device)
            self.fix_cache[f'cuda:{i}'] = (k, v, h, d, id, t)

        # self.select_curve = {'cuda:0':torch.zeros(batch_size_per_device, 0, device=torch.device('cuda:0'))}
    ...
    def update_cache(self, key_states, value_states, hidden_states, doc_ids, device_name, input_ids, timestamp):
        ...
        if self.xl_cache_size > 0 and device_name in self.xl_cache:
            to_fix_cache = [ key_states[:, :, :-self.xl_cache_size].detach(), \
            ...
            to_fix_cache[2] = torch.cat([self.xl_cache[device_name][2], to_fix_cache[2]], dim=1)
            to_fix_cache[3] = torch.cat([self.xl_cache[device_name][3], to_fix_cache[3]], dim=2)
            
            to_fix_cache[4] = torch.cat([self.xl_cache[device_name][4], to_fix_cache[4]], dim=2)
        else:
        ...
        old_k, old_v, old_h, old_d, old_id, old_t = self.fix_cache[device_name]

        if to_fix_cache[2].size(1) != old_h.size(1):
            # print("end of epoch, return")
            return
        
        # def _add_grad(grad):
        #     self.first_select_num[device_name] += grad.sum().item()
        #     self.total_num[device_name] += grad.numel()
        
        # to_cache_diff = torch.ones_like(old_t).float().unsqueeze(-1).clone()
        # to_cache_diff.requires_grad_(True)
        # to_cache_diff.register_hook(_add_grad)
        # self.replacer(torch.cat([to_fix_cache[2], to_cache_diff * self.t_emb.view(1, 1, -1)], dim=-1)).sum().backward()

        # old_t_diff = (timestamp + 1 - old_t.unsqueeze(-1)).float().clone()
        # old_t_diff.requires_grad_(True)
        # old_t_diff.register_hook(_add_grad)
        # self.replacer(torch.cat([old_h, old_t_diff * self.t_emb.view(1, 1, -1)], dim=-1)).sum().backward()
        ...
        with torch.no_grad():
            ...
            log_alpha = self.replacer(torch.cat([to_fix_cache[2], to_cache_diff], dim=-1)) - self.replacer(torch.cat([old_h, old_t_diff], dim=-1))

            indicator = (log_alpha > 0).unsqueeze(1) | (to_fix_cache[3] != old_d).unsqueeze(-1)

            # draw figures:
            # self.total_num[device_name] += (to_fix_cache[3] == old_d)[:, 0, :].sum().item()
            # self.first_select_num[device_name] += ((log_alpha > 0).squeeze(-1) & (to_fix_cache[3] == old_d)[:, 0]).sum().item()
            # self.select_curve[device_name] = torch.cat([ self.select_curve[device_name], indicator[:,0, :, 0].sum(dim=1, keepdim=True) / indicator.size(2)], dim=-1)
            # for i in range(indicator.size(0)):
            #     self.pass_info[f'{i}-to_fix_ids'] = to_fix_cache[4][i, 0, :]
            #     self.pass_info[f'{i}-old_ids'] = old_id[i, 0, :]
            #     self.pass_info[f'{i}-mask'] = indicator[i, 0, :, 0]
            # self.pass_info['to_fix_h'] = to_fix_cache[2]
            # self.pass_info['to_fix_ids'] = to_fix_cache[4]

            # the alternative approach
            # newest_d = to_fix_cache[3][:, 0, -1:]
            # to_fix_penalty = torch.where(to_fix_cache[3][:, 0, :] != newest_d, -100.0, 0.0)
            # old_penalty = torch.where(old_d[:, 0, :] != newest_d, -100.0, 0.0)

            # to_cache_diff = torch.ones_like(old_t).unsqueeze(-1) * self.t_emb.view(1, 1, -1)
            # to_fix_score = self.replacer(torch.cat([to_fix_cache[2], to_cache_diff], dim=-1)).squeeze(-1) + to_fix_penalty

            # old_t_diff = (timestamp + 1 - old_t.unsqueeze(-1)) * self.t_emb.view(1, 1, -1)
            # old_score = self.replacer(torch.cat([old_h, old_t_diff], dim=-1)).squeeze(-1) + old_penalty

            # _, best_idx = torch.topk(torch.cat([old_score, to_fix_score], dim=-1), self.fix_cache_size, dim=-1, sorted=False)
            # best_idx = best_idx.unsqueeze(1).unsqueeze(-1).expand(-1, self.n_heads, -1, self.key_value_proj_dim)

            # new_k = torch.gather(torch.cat([old_k, to_fix_cache[0]], dim=2), dim=2, index=best_idx)
            # new_v = torch.gather(torch.cat([old_v, to_fix_cache[1]], dim=2), dim=2, index=best_idx)
            # new_h = torch.gather(torch.cat([old_h, to_fix_cache[2]], dim=1), dim=1, index=best_idx[:,0,:,0:1].contiguous().expand(-1, -1, self.d_model))
            # new_d = torch.gather(torch.cat([old_d, to_fix_cache[3]], dim=2), dim=2, index=best_idx[:,:,:,0])
            # new_id = torch.gather(torch.cat([old_id, to_fix_cache[4]], dim=2), dim=2, index=best_idx[:,:,:,0])
            # new_t = torch.gather(torch.cat([old_t, timestamp * torch.ones_like(old_t)], dim=1), dim=1, index=best_idx[:,0,:,0])
            # self.fix_cache[device_name] = (new_k, new_v, new_h, new_d, new_id, new_t)   #cloud4 2  3


            if FLAG_TEST and random.randint(0, 3000) == 0:
                ...
                print(' '.join([ f'|<{token1}> {token2}' if chosen>0 else f'|{token1} <{token2}>' for chosen, token1, token2 in zip(log_alpha[BATCH_IDX, :PRINT_LEN, 0].tolist(),\
                                                                                                            TOKENIZER.convert_ids_to_tokens(to_fix_cache[4][BATCH_IDX, 0, :PRINT_LEN].tolist()), \
                                                                                                            TOKENIZER.convert_ids_to_tokens(old_id[BATCH_IDX, 0, :PRINT_LEN].tolist()))]))
        
            new_k = torch.where(indicator, to_fix_cache[0], old_k)
            new_v = torch.where(indicator, to_fix_cache[1], old_v)
            new_h = torch.where(indicator[:,0], to_fix_cache[2], old_h)
            new_d = torch.where(indicator.squeeze(-1), to_fix_cache[3], old_d)
            new_id = torch.where(indicator.squeeze(-1), to_fix_cache[4], old_id)
            new_t = torch.where(indicator[:, 0, :, 0], timestamp, old_t)
        
        self.fix_cache[device_name] = (new_k, new_v, new_h, new_d, new_id, new_t)
    ...
    def forward(
        ...
        if self.fix_cache_size > 0 and device_name in self.fix_cache:
            cache_key, cache_value, cache_hidden, cache_doc_ids, cache_input_ids, cache_timestamp = self.fix_cache[device_name]
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention(config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=has_relative_attention_bias)
...
class T5Block(nn.Module):
    def __init__(self, config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens, block_size, xl_cache_size, fix_cache_size):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, block_size, xl_cache_size, fix_cache_size, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
...
class T5ModelDecoderCacheReplacer(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, block_size, xl_cache_size, fix_cache_size, selector_cache_size, selector_cache_num, tokenizer=None):
        super().__init__(config)

        print("params:\t", "xl_cache", xl_cache_size, "fix_cache", fix_cache_size, "selector", selector_cache_size, selector_cache_num)
        ...
        self.decoder = T5Stack(decoder_config, self.shared, block_size, xl_cache_size, fix_cache_size)
    ...
    def reset_cache(self, batch_size_per_device):
        ...
        for layer_module in self.decoder.block:
            ...
            layer_module.layer[0].SelfAttention.fix_cache.clear()
        self.selector_cache.clear()
        # print(torch.cuda.memory_summary())
        # torch.cuda.empty_cache()
        for layer_module in self.decoder.block:
            layer_module.layer[0].SelfAttention.init_fix_cache(batch_size_per_device)
        ...
        for k, v, h, t in key_values:
            sliced_key_values = sliced_key_values + ((k[:,:, s:s+self.selector_cache_size].contiguous(), \
                                                      ...
                                                      h[:,s:s+self.selector_cache_size].contiguous(),\
                                                        ...
                                                        input_ids[:, s:s+self.selector_cache_size].contiguous(), t),)
    ...
    def forward(
        ...
        if device_name in self.selector_cache and self.training:
