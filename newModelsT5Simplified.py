...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=False):
        super().__init__()
        ...
        self.fix_cache_size = fix_cache_size if fix_cache_size > 0 else 0
        self.new_fix_cache_per_step = new_fix_cache_per_step

        if self.fix_cache_size > 0:
            self.fix_cache_bias = nn.Parameter(torch.zeros(1, self.n_heads, 1, 1))
        ...
        # self.first_select_num = {'cuda:0':0, 'cuda:1':0}
        # self.second_select_num = {'cuda:0':0, 'cuda:1':0}
        # self.total_num = {'cuda:0':0, 'cuda:1':0}
        # self.pass_info = {}
    ...
    def update_cache(self, key_states, value_states, doc_ids, select_mask, device_name, input_ids):
        ...
        if self.fix_cache_size > 0:
            if self.xl_cache_size > 0:
                ...
                to_fix_cache = [ key_states[:, :, :-self.xl_cache_size].contiguous().detach(), \
                ...
                to_fix_cache = [ key_states.detach(), \
                                        value_states.detach(), \
                                        doc_ids, select_mask.detach(), input_ids ]
            if self.xl_cache_size > 0 and device_name in self.xl_cache:
                # add if exists
                to_fix_cache[0] = torch.cat([self.xl_cache[device_name][0], to_fix_cache[0]], dim=2)
                to_fix_cache[1] = torch.cat([self.xl_cache[device_name][1], to_fix_cache[1]], dim=2)
                to_fix_cache[2] = torch.cat([self.xl_cache[device_name][2], to_fix_cache[2]], dim=2)
                to_fix_cache[3] = torch.cat([self.xl_cache[device_name][3], to_fix_cache[3]], dim=-1)
                
                to_fix_cache[4] = torch.cat([self.xl_cache[device_name][4], to_fix_cache[4]], dim=2) if input_ids is not None else None


            # get inital cache
            if device_name in self.fix_cache:
                old_k, old_v, old_d, old_id = self.fix_cache[device_name]
            else:
                shape = key_states.size()[:2] + (self.fix_cache_size,) + key_states.size()[3:4]
            ...
            # select and add
            if self.new_fix_cache_per_step == 0:    # the length is not the same for each batch_idx
                m = to_fix_cache[3]
                m = m > self.alpha_thr
                # self.total_num[device_name] += (to_fix_cache[0].size(0) * to_fix_cache[0].size(2))
                k = to_fix_cache[0].transpose(1,2)[m].split(m.sum(dim=1).tolist(), dim=0)
                v = to_fix_cache[1].transpose(1,2)[m].split(m.sum(dim=1).tolist(), dim=0)
                d = to_fix_cache[2].transpose(1,2)[m].split(m.sum(dim=1).tolist(), dim=0)
                
                id = to_fix_cache[4].transpose(1,2)[m].split(m.sum(dim=1).tolist(), dim=0) if input_ids is not None else None
                for i in range(key_states.size(0)):
                    ...
                    if self.rmD:
                        ...
                        threshold = dist.mean().item() / 2    # 1.5 2
                        mask = (dist < threshold).float().tril(diagonal=-1).sum(dim=0) == 0

                        # self.pass_info[f'{i}-ids'] = to_fix_cache[4][i, 0, :]
                        # self.pass_info[f'{i}-mask1'] = m[i, :]
                        # self.pass_info[f'{i}-mask2'] = mask
                        # assert m[i, :].sum() == mask.size(0)

                        # self.first_select_num[device_name] += add_len
                        if FLAG_TEST and random.randint(0,1000) == 0:
                    ...
                    # dist2 = torch.cdist(old_v[i].transpose(0, 1).contiguous().view(-1, self.inner_dim), v[i][mask].view(-1, self.inner_dim))
                    # mask2 = (dist2 < threshold).sum(dim=1) == 0

                    # add_len = max(mask.sum().item() - (self.fix_cache_size - mask2.sum().item()), 0)
                    # if add_len < mask2.sum().item():
                    #     new_k = torch.cat([old_k[i].transpose(0,1)[mask2][add_len:], k[i][mask]], dim=0).transpose(0, 1)
                    #     old_k[i, :, -new_k.size(1):] = new_k
                    #     new_v = torch.cat([old_v[i].transpose(0,1)[mask2][add_len:], v[i][mask]], dim=0).transpose(0, 1)
                    #     old_v[i, :, -new_v.size(1):] = new_v
                    #     new_d = torch.cat([old_d[i].transpose(0,1)[mask2][add_len:], d[i][mask]], dim=0).transpose(0, 1)
                    #     old_d[i, :, -new_d.size(1):] = new_d
                    #     if input_ids is not None:
                    #         new_id = torch.cat([old_id[i].transpose(0,1)[mask2][add_len:], id[i][mask]], dim=0).transpose(0, 1)
                    #         old_id[i, :, -new_id.size(1):] = new_id
                    # else:
                    #     old_k[i] = k[i][mask].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                    #     old_v[i] = v[i][mask].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                    #     old_d[i] = d[i][mask].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                    #     if input_ids is not None:
                    #         old_id[i] = id[i][mask].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                    # ===========================================
                        add_len = mask.sum().item()
                        # self.second_select_num[device_name] += add_len
                        if add_len < self.fix_cache_size:
                        ...
                        if add_len < self.fix_cache_size:
                            ...
                            old_k[i] = k[i].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                            old_v[i] = v[i].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                            old_d[i] = d[i].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
                            if input_ids is not None:
                                old_id[i] = id[i].transpose(0, 1)[:, -self.fix_cache_size:].contiguous()
            else:
                if self.new_fix_cache_per_step < to_fix_cache[0].size(2):
                    # _, idx = torch.topk(to_fix_cache[3], self.new_fix_cache_per_step, dim=2, sorted=False)
                    # idx = idx.unsqueeze(-1).expand(-1, -1, -1, key_states.size(-1))
                    _, idx = torch.topk(to_fix_cache[3], self.new_fix_cache_per_step, dim=1, sorted=False)
                    idx = idx.unsqueeze(-1).unsqueeze(1).expand(-1, key_states.size(1), -1, key_states.size(-1))
                    k = torch.gather(to_fix_cache[0], dim=2, index=idx)
                    v = torch.gather(to_fix_cache[1], dim=2, index=idx)
                    d = torch.gather(to_fix_cache[2], dim=2, index=idx[:, :, :, 0])

                    id = torch.gather(to_fix_cache[4], dim=2, index=idx[:, :, :, 0]) if input_ids is not None else None

                    add_len = self.new_fix_cache_per_step
                else:
                    k = to_fix_cache[0]
                    v = to_fix_cache[1]
                    d = to_fix_cache[2]

                    id = to_fix_cache[4] if input_ids is not None else None

                    add_len = to_fix_cache[0].size(2)

                if add_len < self.fix_cache_size:
                    old_k = torch.cat([old_k[:, :, add_len:], k], dim=2)
                    old_v = torch.cat([old_v[:, :, add_len:], v], dim=2)
                    old_d = torch.cat([old_d[:, :, add_len:], d], dim=2)
                    if input_ids is not None:
                        old_id = torch.cat([old_id[:, :, add_len:], id], dim=2)
                else:
                    old_k = k[:,:,-self.fix_cache_size:].contiguous()
                    old_v = v[:,:,-self.fix_cache_size:].contiguous()
                    old_d = d[:,:,-self.fix_cache_size:].contiguous()
                    if input_ids is not None:
                        old_id = id[:,:,-self.fix_cache_size:].contiguous()

            self.fix_cache[device_name] = (old_k, old_v, old_d, old_id)
    ...
    def forward(
        ...
        if self.fix_cache_size > 0 and device_name in self.fix_cache:
            cache_key, cache_value, cache_doc_ids = self.fix_cache[device_name][:3]
            # print("fix_cache", device_name, cache_key.size())
            key_states = torch.cat([cache_key, key_states], dim=2)
            value_states = torch.cat([cache_value, value_states], dim=2)
            doc_ids = torch.cat([cache_doc_ids, doc_ids], dim=2)

            if input_ids is not None:
                input_ids = torch.cat([self.fix_cache[device_name][-1], input_ids], dim=2)
        ...
        fix_key_num = 0
        if self.fix_cache_size > 0 and device_name in self.fix_cache:
            fix_key_num += self.fix_cache_size
        if past_kv is not None:
            fix_key_num += past_d.size(-1)
        
        if fix_key_num > 0:
            scores += torch.cat([self.fix_cache_bias.expand(-1, -1, -1, fix_key_num).contiguous(), \
                   torch.zeros((1, self.n_heads, 1, scores.size(-1) - fix_key_num,), device=scores.device, dtype=scores.dtype)], dim=-1)

        if position_bias is None:
            assert self.has_relative_attention_bias
            
            position_bias = self.compute_bias(query_states.size(2), key_states.size(2), fix_key_num, device=scores.device)
        ...
        # print(device_name, current_key_states.size())
        with torch.no_grad():
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention(config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=has_relative_attention_bias)
...
class T5Block(nn.Module):
    def __init__(self, config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, use_selector, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
...
class T5ModelDecoderCacheSelector(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, block_size, xl_cache_size, fix_cache_size, threshold, new_fix_cache_per_step, selector_cache_size, selector_cache_num, L0_lambda, rmD, tokenizer=None):
        super().__init__(config)

        print("params:\t", "xl_cache", xl_cache_size, "fix_cache", fix_cache_size, new_fix_cache_per_step, "selector", selector_cache_size, selector_cache_num, L0_lambda, rmD)

        self.block_size = block_size
        self.xl_cache_size = xl_cache_size
        self.fix_cache_size = fix_cache_size
        self.threshold = threshold
        self.use_selector = selector_cache_num > 0
        ...
        self.decoder = T5Stack(decoder_config, self.shared, block_size, xl_cache_size, fix_cache_size, new_fix_cache_per_step, rmD, self.use_selector)
        ...
        self.selector_cache = {}
        self.selector_cache_size = selector_cache_size
        self.selector_cache_num = selector_cache_num
        ...
        if FLAG_TEST and tokenizer is not None:
            ...
            global MASKED_VOCAB
            MASKED_VOCAB = torch.tensor(masked_vocab)
    ...
    def reset_cache(self, batch_size_per_device=None):
        ...
        for layer_module in self.decoder.block:
            ...
            layer_module.layer[0].SelfAttention.fix_cache = {}
        self.selector_cache = {}

    def update_selector_cache(self, key_values, doc_ids, input_ids):
        device_name = str(doc_ids.device)
        if doc_ids.size(1) > self.selector_cache_size:
            s = torch.randint(0, doc_ids.size(1) - self.selector_cache_size + 1, size=(1,)).item()
        else:
            s = 0
        
        sliced_key_values = ()
        for k, v, h in key_values:
            sliced_key_values = sliced_key_values + ((k[:,:, s:s+self.selector_cache_size].contiguous(), \
                                                      v[:,:, s:s+self.selector_cache_size].contiguous(), \
                                                      h[:,s:s+self.selector_cache_size].contiguous(),\
                                                        doc_ids[:, s:s+self.selector_cache_size].contiguous(), \
                                                        input_ids[:, s:s+self.selector_cache_size].contiguous()), )

        if device_name in self.selector_cache:
            if len(self.selector_cache[device_name]) == self.selector_cache_num:    # need to replace
                idx = torch.randint(0, self.selector_cache_num, size=(1,)).item()
                self.selector_cache[device_name][idx] = sliced_key_values
            elif len(self.selector_cache[device_name]) < self.selector_cache_num:
                self.selector_cache[device_name].append(sliced_key_values)
            else:
                raise RuntimeError
        else:
            self.selector_cache[device_name] = [sliced_key_values]
    ...
    def forward(
        ...
        if self.use_selector and device_name in self.selector_cache and self.training:
            idx = torch.randint(0, len(self.selector_cache[device_name]), size=(1,)).item()
            past_key_values = self.selector_cache[device_name][idx]
        else:
            past_key_values = None

        if self.fix_cache_size > 0 and not self.use_selector and fix_duration is not None:
            # fix_duration = torch.randint_like(fix_duration, low=0, high=12)
            # fix_duration = torch.cat([fix_duration[:, 1:], 
            #                           torch.zeros(fix_duration.size(0), 1, device=fix_duration.device, dtype=fix_duration.dtype)], dim=-1)
        ...
        if self.use_selector and self.training:
            self.update_selector_cache(decoder_outputs.present_key_values, doc_ids, input_ids)
